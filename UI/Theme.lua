...
local GoldTrackerTheme = {}
GoldTrackerTheme.__index = GoldTrackerTheme
...
function GoldTrackerTheme:New(options)
    options = type(options) == "table" and options or {}

    local instance = {
        addon = options.addon,
        backdrop = options.backdrop or DEFAULT_BACKDROP,
        colors = options.colors or DEFAULT_COLORS,
        buttonPalettes = options.buttonPalettes or DEFAULT_BUTTON_PALETTES,
    }

    return setmetatable(instance, self)
end

function GoldTrackerTheme:GetColor(colorOrKey, fallback)
...
function GoldTrackerTheme:GetPalette(paletteKey)
    return self.buttonPalettes[paletteKey] or self.buttonPalettes.neutral
end

function GoldTrackerTheme:ApplyBackdrop(frame, bg, border)
...
function GoldTrackerTheme:CreatePanel(parent, bg, border)
    local panel = CreateFrame("Frame", nil, parent, "BackdropTemplate")
    self:ApplyBackdrop(panel, bg, border)
    return panel
end

function GoldTrackerTheme:UpdateButtonVisual(button)
...
function GoldTrackerTheme:CreateButton(parent, width, height, text, paletteKey)
...
function GoldTrackerTheme:HideNativeChrome(frame)
    if not frame then
        return
    end
...
function GoldTrackerTheme:ApplyWindowChrome(frame, titleText, options)
    if not frame then
        return nil, nil
    end
    ...
    local closeButton = self:CreateButton(headerBar, options.closeWidth or 22, options.closeHeight or 22, options.closeText or "X", "neutral")
...
function GoldTrackerTheme:RegisterSpecialFrame(frameName)
...
function GoldTrackerTheme:BringToFront(frame, relativeFrame)
    if not frame then
        return
    end
...
NS.GoldTrackerThemeClass = GoldTrackerTheme
NS.GoldTrackerTheme = NS.GoldTrackerTheme or GoldTrackerTheme:New({ addon = NS.GoldTracker })
